...
static void pollDisplayState() {
  ...
  else if (strcmp(dState, "proactive") == 0) {
    tftSetStateWithData(STATE_PROACTIVE, dp); currentState = STATE_PROACTIVE;
    // Auto-play if backend provides a TTS URL (talkative_delivery: auto_speak)
    const char* ttsUrl = doc["tts_url"] | "";
    if (strlen(ttsUrl) > 0) {
      Serial.printf("[PROACTIVE] Auto-playing TTS: %s\n", ttsUrl);
      currentState = STATE_PLAYING;
      tftSetState(STATE_PLAYING);
      audioPlayerPlay(String(ttsUrl));
    }
  }

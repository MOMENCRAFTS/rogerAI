...
export default function SalahView({ location }: Props) {
  ...
  // ── Prayer Entry Announcement state ──────────────────────────────────────
  const [announcedToday, setAnnouncedToday] = useState<Record<string, boolean>>({});
  ...
  const loadData = useCallback(async (isRefresh = false) => {
    ...
    setError(null);
    try {
      const pTimes = await fetchPrayerTimes(stableLat, stableLng);
      setTimes(pTimes);
  ...
  // ── Countdown ticker ──────────────────────────────────────────────────────
  useEffect(() => {
    timerRef.current = setInterval(() => setQibla(q => q), 1000);
    return () => { if (timerRef.current) clearInterval(timerRef.current); };
  }, []);

  // ── Prayer Entry TTS Announcement ─────────────────────────────────────────
  useEffect(() => {
    if (!times) return;
    const ANNOUNCE_PRAYERS = ['Fajr', 'Dhuhr', 'Asr', 'Maghrib', 'Isha'] as const;
    const now = new Date();
    const nowSecs = now.getHours() * 3600 + now.getMinutes() * 60 + now.getSeconds();

    for (const name of ANNOUNCE_PRAYERS) {
      if (announcedToday[name]) continue;
      const [h, m] = times[name].split(':').map(Number);
      const prayerSecs = h * 3600 + m * 60;
      const diff = nowSecs - prayerSecs;
      // Fire if within ±60 seconds of prayer time
      if (diff >= 0 && diff <= 60) {
        // Mark as announced
        setAnnouncedToday(prev => ({ ...prev, [name]: true }));
        // TTS voice announcement
        if ('speechSynthesis' in window) {
          const utter = new SpeechSynthesisUtterance(`It is time for ${name} prayer`);
          utter.rate = 0.9;
          utter.volume = 1;
          window.speechSynthesis.speak(utter);
        }
        // Visual banner
        setAnnounceBanner(name);
        if (bannerTimeout.current) clearTimeout(bannerTimeout.current);
        bannerTimeout.current = setTimeout(() => setAnnounceBanner(null), 8000);
        break; // one at a time
      }
    }
  }); // runs every re-render (driven by 1s ticker)

  // Reset daily announcements at midnight
  useEffect(() => {
    const now = new Date();
    const msToMidnight = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1).getTime() - now.getTime();
    const t = setTimeout(() => setAnnouncedToday({}), msToMidnight);
  ...
  return (
    <div style={{ padding: '0 0 80px', background: 'var(--bg-primary)', minHeight: '100%', position: 'relative' }}>
      ...
      {/* ── Prayer Entry Announcement Banner ── */}
      {announceBanner && (
        <div style={{
          ...
          <div>
            <p style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 700, color: EMERALD, margin: '0 0 2px', letterSpacing: '0.06em' }}>
              It's time for {announceBanner}
            </p>
            <p style={{ fontFamily: 'monospace', fontSize: 9, color: 'var(--text-muted)', margin: 0, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
      ...
      <div style={{ padding: '0 16px', position: 'relative', zIndex: 1 }}>
        ...
        {next && (
          <div style={{
            marginBottom: 20, background: EMERALD_DIM,
            border: `1px solid ${EMERALD}30`, padding: '24px 20px',
            position: 'relative', overflow: 'hidden',
          }}>
            <div style={{ position: 'absolute', right: 20, top: '50%', transform: 'translateY(-50%)', width: 72, height: 72 }}>
              <svg width="72" height="72" viewBox="0 0 72 72" style={{ overflow: 'visible' }}>
                ...
                />
                <text x="36" y="40" textAnchor="middle" fill={EMERALD} fontSize="11" fontFamily="monospace" fontWeight="700">☽</text>
              </svg>
            </div>
        ...
        {(() => {
          ...
          return (
            <div id="prayer-time-remaining" style={{
              marginBottom: 16, background: bgColor,
              border: `1px solid ${borderClr}`, padding: '10px 14px',
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 4 }}>
                  <span style={{ fontFamily: 'monospace', fontSize: 11, fontWeight: 700, color: barColor, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                    {curName}
                  </span>
                  <span style={{ fontFamily: 'monospace', fontSize: 10, color: isUrgent ? '#fca5a5' : isWarning ? '#fcd34d' : 'var(--text-muted)' }}>
                    ends in <strong>{formatCountdown(secondsLeft)}</strong>
                  </span>
                  {isUrgent && (
                    <span style={{ fontFamily: 'monospace', fontSize: 8, color: '#ef4444', textTransform: 'uppercase', letterSpacing: '0.15em', fontWeight: 700, animation: 'pulse-text 1.5s ease-in-out infinite' }}>
                      Don't miss it!
                    </span>
                  )}
                  {!isUrgent && isWarning && (
                    <span style={{ fontFamily: 'monospace', fontSize: 8, color: '#f59e0b', textTransform: 'uppercase', letterSpacing: '0.12em' }}>
                      Ending soon
                    </span>
                  )}
                </div>
                <div style={{ width: '100%', height: 3, background: 'rgba(255,255,255,0.06)', borderRadius: 2, overflow: 'hidden' }}>
                  <div style={{ width: `${pct}%`, height: '100%', background: barColor, borderRadius: 2, transition: 'width 1s linear, background 400ms' }} />
                </div>
              </div>
            </div>
          );
            ...
            return (
              <div key={name} style={{
                ...
                <span style={{
                  ...
                  fontWeight: isNext ? 700 : 400, flex: 1, letterSpacing: '0.06em',
                }}>
                  {name}
                </span>
                {isNext && (
        ...
        <div style={{
          marginBottom: 20, background: EMERALD_DIM,
          border: `1px solid ${EMERALD}25`, padding: '24px 20px',
          ...
          <div style={{ position: 'relative', width: 200, height: 200 }}>
            <svg width="200" height="200" viewBox="0 0 200 200" style={{ position: 'absolute', inset: 0 }}>
              ...
              {[0, 45, 90, 135, 180, 225, 270, 315].map(deg => {
                ...
                return (
                  <line key={deg}
                    ...
                    stroke={`${EMERALD}${isMajor ? '70' : '40'}`} strokeWidth={isMajor ? 2 : 1}
                  />
                );
              })}
              {[
              ...
              <g transform={`rotate(${needleAngle}, 100, 100)`} style={{ transition: compassSupported ? 'transform 0.2s ease' : 'none' }}>
                <polygon points="100,18 95,100 100,104 105,100" fill={EMERALD} opacity="0.9" />
                <polygon points="100,182 95,100 100,104 105,100" fill={`${EMERALD}30`} />
                <rect x="94" y="12" width="12" height="10" rx="1" fill={EMERALD} opacity="0.95" />
                <text x="100" y="20" textAnchor="middle" fontSize="6" fill="#0a0a0a" fontWeight="700">🕋</text>
              </g>
            </svg>
          </div>
